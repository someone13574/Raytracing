...
namespace Graphics
{
	...
	public:
		void UpdateCameraToWorldMatrix()
		{
			...
			cw.Normalize();
			Vector3<double> cu = Vector3<double>::Cross(cw, Vector3<double>(0, 1, 0)); //cw, (0, 1, 0)
			cu.Normalize();
			Vector3<double> cv = Vector3<double>::Cross(cu, cw); //cu, cw
	...
	public:
		...
		Vector3<double> position = Vector3<double>(2.5, 2, 0);
	};
}